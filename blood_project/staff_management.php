...
$res = mysqli_query($conn, "SELECT fullname, username, role FROM staff ORDER BY role ASC");
?>
...
<head>
    ...
    <title>AI Security | Staff Authentication Logs</title>
    ...
    <style>
        ...
        .data-panel::before {
            content: "ENCRYPTED_PERSONNEL_FILE";
            position: absolute;
            top: 10px;
            right: 20px;
            font-size: 9px;
        ...
        .role-admin { background: rgba(0, 255, 157, 0.1); color: var(--neon-emerald); border: 1px solid var(--neon-emerald); }
        .role-doctor { background: rgba(76, 201, 240, 0.1); color: var(--ai-blue); border: 1px solid var(--ai-blue); }
        .role-staff { background: rgba(136, 146, 176, 0.1); color: #8892b0; border: 1px solid #8892b0; }

        .username-text { color: var(--ai-blue); opacity: 0.8; font-size: 13px; }
    </style>
</head>
<body>
    ...
    <div class="sidebar">
        ...
        <a href="staff_management.php" style="color:var(--neon-emerald);">● AUTH: STAFF_LIST</a>
    </div>
    ...
    <div class="main-content">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Terminal_User</th>
                        <th style="text-align:right;">Clearance_Level</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    while($row = mysqli_fetch_assoc($res)) {
                        ...
                        if($row['role'] == 'Doctor') $roleClass = 'role-doctor';

                        echo "<tr>
                                <td style='font-weight:500; color:#fff;'>".htmlspecialchars($row['fullname'])."</td>
                                <td class='username-text'>_".htmlspecialchars($row['username'])."</td>
        ...
        <div style="margin-top: 40px; display: flex; gap: 20px; opacity: 0.7;">
            <div style="font-size: 10px;">ENCRYPTION: AES-256</div>
            <div style="font-size: 10px;">Uptime: 99.98%</div>
            <div style="font-size: 10px;">Session: Active</div>
        </div>
    </div>
